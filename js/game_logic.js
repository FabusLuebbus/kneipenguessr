import { getUserLocation, getDistanceFromLatLonInKm } from "./location_logic.js";
...
function loadNextBar(closestBar) {
  ...
  // Update the DOM
  document.getElementById("bar1").querySelector(".bar_name").innerText = bar1.name;
  document.getElementById("bar2").querySelector(".bar_name").innerText = bar2.name;
  document.getElementById("bar1").querySelector(".bar_distance").innerText = bar1.distance.toFixed(2) + " km";
  document.getElementById("bar2").querySelector(".bar_distance").innerText = bar2.distance.toFixed(2) + " km";
  countdown.reset();
  countdown.start();
}
...
async function initializeGame() {   
        barsArray = shuffleArray(barData.map(bar => ({
            name: bar.name,
            distance: getDistanceFromLatLonInKm(userLatitude, userLongitude, bar.coordinates.latitude, bar.coordinates.longitude),
        })));
        ...
        document.getElementById("bar1").querySelector(".bar_distance").innerText = bar1.distance.toFixed(2) + " km";
        document.getElementById("bar2").querySelector(".bar_distance").innerText = bar2.distance.toFixed(2) + " km";
        countdown = new CountdownTimer("timer1", 10, () => showGameOver("Time's up!"));
        countdown.start();
}
